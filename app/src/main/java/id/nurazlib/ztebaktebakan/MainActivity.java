...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadNextQuestion() {
        if (currentQuestionIndex < questions.size() && currentQuestionIndex < maxLevel) {
            Question currentQuestion = questions.get(currentQuestionIndex);
            questionText.setText(currentQuestion.getQuestionText());
            levelText.setText("Level " + (currentQuestionIndex + 1));
            ...
            // Tampilkan iklan interstisial setiap 5 level (mulai dari level 5)
            if (mInterstitialAd != null && (currentQuestionIndex + 1) % 5 == 0 && currentQuestionIndex != 0) {
                mInterstitialAd.show(MainActivity.this);
            }
        } else {
            endGame();
